...
def list_jobs(
    ...
    q = db.query(ScrapeJob).filter(ScrapeJob.user_id == user_id)
    if status:
        q = q.filter(ScrapeJob.status == status)

    total = q.count()
    jobs = q.order_by(ScrapeJob.created_at.asc()).offset(offset).limit(limit).all()
...
@router.post("/jobs/{job_id}/claim", response_model=ScrapeJobOut)
def claim_job(job_id: int, user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
    rows = db.query(ScrapeJob).filter(
        ScrapeJob.id == job_id,
    ...
    db.commit()
    if rows == 0:
        raise HTTPException(status_code=409, detail="Job not available (already claimed or completed)")
